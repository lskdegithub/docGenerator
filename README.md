...
### 表格格式
- **跨页表格**：使用 `longtable` 环境支持大表格跨页，保留multirow的一对多关系
- **表头重复**：每页自动重复表头，表头使用灰色背景 `\rowcolor{gray!20}`
- **分页闭合**：使用 `endfoot` 确保multirow跨页时单元格边框完整闭合
- **行高自适应**：根据内容长度动态调整最后一行的行高，避免穿模
...
**v3.2 更新内容**：
- ✨ 新增：1.2章节表格自动生成功能（覆盖性对照表）
- ✨ 新增：longtable支持，实现表格跨页显示
- ✨ 新增：multirow跨页边框自动闭合
- ✨ 新增：动态行高调整，根据内容长度自适应避免穿模
- 🐛 修复：YAML多行内容解析错误
- 🐛 修复：4.2章节三级标题只显示标题，不显示content内容
- 📝 更新：gjb438c-style.sty 添加longtable和multirow包支持
