...
**v3.3 更新内容**：
- ✨ 新增：第7章表格自动生成功能（需求追踪表），支持双向追踪
- ✨ 优化：升级表格引擎为 `tabularray` (longtblr)，彻底解决跨页断行、竖线溢出和文字穿模问题
- ✨ 优化：实现长文本智能切分与动态测试项分配算法，确保页面布局美观
- 🐛 修复：解决表格长文本导致的内容重叠和分页错误
- 📝 更新：`build_test_plan.sh` 脚本优化，支持自动化构建全流程
